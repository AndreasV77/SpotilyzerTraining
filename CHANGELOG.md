...
## 2026-07-13
- DOC_AUDIT.md — added a "Repo hygiene" section to the (still read-only) audit pass: branch attachment, local/origin sync state, tracked-file/.gitignore gaps, working-tree cleanliness. Motivation: an accidental local deletion followed by "recovery" from the remote silently resurrects whatever bad git state the remote has (detached HEAD, wrongly-tracked binaries, unpushed work) — exactly what this session hit (detached HEAD, git-tracked archive/ joblibs). Findings logged to the same ERRATA.md, same ID scheme. Kept identical in both repos.
